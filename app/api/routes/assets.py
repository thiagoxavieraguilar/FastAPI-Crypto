...
async def create_user(favorite_input: FavoriteInput, currentUser = Depends(token_verifier), service: FavoriteService = Depends(get_favorite_repository)):
    try:
        ...
        favorite = service.create_favorite(symbol=favorite_input.symbol.upper(), user_id=user_id)
...
@assets.get('/show_favorites', description='My description', response_model=List[PriceFavorite], responses={400: {'model': ErrorOutput}})
async def remove_favorite(currentUser = Depends(token_verifier), service: FavoriteService = Depends(get_favorite_repository), asset_service: AssetService = Depends(get_assets_service)):
    try:
        user_id = currentUser['id']
        ...
        tasks = [asset_service.get_price(symbol=favorite['symbol']) for favorite in favorites]
